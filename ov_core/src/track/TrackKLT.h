...
public:
   ...
   * @param record_parallax if we should record parallax information to a CSV file
   */
  explicit TrackKLT(std::unordered_map<size_t, std::shared_ptr<CamBase>> cameras, int numfeats, int numaruco, bool stereo,
                    HistogramMethod histmethod, int fast_threshold, int gridx, int gridy, int minpxdist, bool record_parallax = false)
      : TrackBase(cameras, numfeats, numaruco, stereo, histmethod), threshold(fast_threshold), grid_x(gridx), grid_y(gridy),
        min_px_dist(minpxdist), record_parallax_to_file(record_parallax) {}
...
protected:
  ...
  // If we should record parallax information to file
